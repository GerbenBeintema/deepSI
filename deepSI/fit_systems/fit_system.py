...
class System_torch(System_fittable):
    ...
    def fit_combined(self, sys_data, epochs=30, batch_size=256, loss_kwargs={}, \
        optimizer_kwargs={}, sim_val=None, verbose=1, cuda=False, val_frac=0.2, sim_val_fun='NRMS', sqrt_train=True, concurrent_val=False):
        '''The batch optimization method with parallel validation, (use if __name__=='__main__' or import from a file if using self defined method)
        ...
        try:
            ...
            for epoch in (tqdm(range(epochs)) if verbose>0 else range(epochs)):
                ...
                if verbose>0:
                    ...
                    Loss_str = f'Epoch {epoch+1:4}, Training loss {train_loss_epoch:7.4}, Validation {val_str} {Loss_val_now:6.4}'
        ...
        self.checkpoint_save_system(name='_last')
        self.checkpoint_load_system()
        if verbose: print(f'Loaded model with best known validation {val_str:6.4} of {self.bestfit} which happened on epoch {best_epoch} (epoch_id={self.epoch_id[-1] if len(self.epoch_id)>0 else 0:.2f})')
